...
import org.json4s.{Formats, JNothing, JValue, ShortTypeHints}
import rules.{RuleElement, RuleGroup}
...
trait JsonHelper extends {
  ...
  implicit val formats: Formats = Serialization.formats(ShortTypeHints(List(classOf[RuleGroup], classOf[RuleElement])))
